...
from .UrlProgressBar import UrlProgressBar
...
def slugify(obj):
    ...
    file_id = obj.get('pdf_url').split('/')[-1]
    file_title = '_'.join(re.findall(r'\w+', obj.get('title', 'UNTITLED')))
    # Prepend object id
    filename = "%s.%s" % (file_id, file_title)
    return filename
...
def download(obj, dirpath='./', slugify=slugify, verbose=True):
    ...
    reporthook = UrlProgressBar() if verbose else None
    urlretrieve(obj['pdf_url'], path, reporthook)
    return path